...
public class GraphAlgorithms {
    ...
    public static <T> Set<Edge<T>> prims(Vertex<T> start, Graph<T> graph) {
        ...
        for (VertexDistance<T> vd : graph.getAdjList().get(start)){
            Edge<T> e = new Edge<>(start, vd.getVertex(), vd.getDistance());
            PQ.add(e);
        }   

        VS.add(start);

        while (!PQ.isEmpty() && VS.size() < numVertices){
            Edge<T> e = PQ.remove();

            Vertex<T> u = e.getU();
            Vertex<T> v = e.getV();

            Edge<T> revE = new Edge<>(v, u, e.getWeight());
            ...
            if (!VS.contains(v)){
                ...
                for (VertexDistance<T> vd : graph.getAdjList().get(v)){
                    Edge<T> next = new Edge<>(v, vd.getVertex(), vd.getDistance());
                    PQ.add(next);
                }
            }
